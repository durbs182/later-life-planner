...
import {
  RLSS, DEFAULT_ASSUMPTIONS, STATE_PENSION,
...
export function buildDefaultIncome(currentAge: number): PersonIncomeSources {
  return {
    ...
    dcPension:    { enabled: false, totalValue: 0, drawdownAge: 65, growthRate: DEFAULT_ASSUMPTIONS.INVESTMENT_GROWTH },
...
export function createDefaultState(primaryAge = DEFAULT_ASSUMPTIONS.DEFAULT_AGE): PlannerState {
  return {
    currentStep: 0,
    mode: 'single',
